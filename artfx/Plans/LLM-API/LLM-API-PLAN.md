...
- **Direct Environment Access**: API keys must be set in the system environment where the server is running.
- **No Local Files**: The application will not use `.env` files or `python-dotenv` for this phase of integration to maintain simplicity and security.
...
- [ ] Ensure all required API keys are exported as system environment variables (e.g., `export AIMLAPI_API=your_key`).
- [ ] Verify access to these keys in a simple Python shell.
